...
| `false` | `duplicate-slug` | **Do not build, and do not move a card.** This folder is a second capture of a business that already has a canonical folder, named in `supersededBy`. The prospect's live mockup is `/<supersededBy>/`. Building here would give one prospect two URLs and split the outreach. No Trello card points at this slug. |

**Before treating any `ready:false` folder as a build, check it is not a
duplicate.** Two captures of one business under different slugs is the most
common cause of a "never built" folder, and the state string will not say so.
The check is two comparisons and it is decisive: same `facebook` handle (or
same `site`) **and** a byte-identical logo file (`md5sum`) means one business,
two folders. Then confirm which is canonical — the one with an `index.html`
that is live — and mark the other `duplicate-slug` with `supersededBy`. Mark,
do not delete: nothing is deployed at the duplicate slug so marking moves no
URL, the capture provenance survives, and removing folders is a human call.
...
- 2026-09-08 (step 2, strategy — **rdap.org returned an empty body, not a 404, for every TLD today**): the standing note says `.ph` has no RDAP because rdap.org 404s it. Today rdap.org returned a **zero-length body** for `.com` and `.co.uk` too — domains that unquestionably have RDAP. An empty body parses as a JSON error and reads exactly like "this TLD has no RDAP service", so trusting rdap.org would have wrongly recorded four domains as unresolvable and skipped the BD-01 registration-date gate on all of them. Go direct to the authoritative server instead: `rdap.verisign.com/com/v1/domain/<d>` and `rdap.nominet.uk/uk/domain/<d>` both answered instantly with registration, expiry and status. Keep rdap.org only as a discovery convenience, never as the verdict, and never read "no output" as "no RDAP".
- 2026-09-08 (step 2 — **the `.ph` wildcard also swallows domains you find in a live MX record**): the known trap is that a *guessed* `.com.ph` resolves via the registry wildcard. The new shape is that `metrodental.com.ph` publishes a real MX record, `mx1.metrodental2.com.ph` at priority 5, and that host resolves to 45.79.222.138 — which is the wildcard. So the "second domain" that looked like a duplicate site of theirs (and was half-written up as a contradictory-listings angle) does not exist at all; the clinic's own mail config points a fallback MX at the registry catch-all. A hostname pulled out of live DNS still needs the wildcard control run against it. The control is two commands: resolve a deliberately absurd `.com.ph` name, and compare IPs.
- 2026-09-08 (step 2 — **a 2.7 KB 200 is a React shell, and the good angle is in the shell's own meta tags**): the standing rule sends you to `/assets/index-*.js` when a lead's site is an SPA, and that is right — the 16 booking buttons and the phone number were only in the 698 KB bundle. But the shell HTML itself carried the sharper finding: `og:url` and `og:image` both pointed at `https://metrodental.vercel.app/`, the platform preview domain, which is live and serves the same page. Their own site therefore tells Facebook, Viber and every crawler that its canonical address belongs to Vercel. Read the shell's meta block before diving into the bundle — a wrong `og:url` is a name-collision angle that the owner can verify by pasting their own URL into a Facebook composer.
- 2026-09-08 (step 2 — **the strongest contradiction was a directory's typo in an email domain, and NXDOMAIN is what proves it**): NHS.uk publishes Doncaster Dental Centre's contact as `reception@docnasterdental.co.uk`. Their real address is `reception@doncasterdental.co.uk`. The transposition is invisible on the page and would have been copied straight onto a lead card as fact; what settles it is that `docnasterdental.co.uk` answers NXDOMAIN on A, MX and NS, so every patient who mails the NHS listing bounces. Two lessons: run the four-command routine on the *email* host, not just the site host, whenever a directory publishes one — and treat a one-character difference between two directory copies of an address as a check to run, not a rendering artifact.
- 2026-09-08 (step 2 — **hours contradicted on the two days the rule predicts, and the practice's own page had none at all**): three sources, three different weeks for one UK practice. NHS.uk: Mon/Tue/Thu/Fri 9:00–5:30 with **Wed 9:00–6:30**. alldentists.co.uk: Mon–Fri **09:00–13:00 and 14:00–17:30** — a daily lunchtime closure NHS.uk does not show anywhere. A third listing ended Friday at 15:00. Their own practice page publishes no opening times whatsoever, which is why the directories are the only record and why they drifted apart unchecked. Wednesday and Friday are the days that differ, on both the UK lead this week and the intl lead yesterday — check those two first and the day-by-day sweep usually pays out inside two comparisons.
- 2026-09-08 (step 2 — **four "book now" surfaces, four different ways of not being a booking**): the standing rule is that a Request Appointment form with a callback promise does not retire `CL-01`. Today's four leads show the rule generalises further than the form-with-a-promise case. Affinity: 9 buttons → a WPForms "Appointment Form" that asks the patient to pick a date and time, which WPForms cannot check against availability, so a human confirms or moves every one. Ora: `/book-now` is headed "Book Your Appointment" and contains **zero `<form>` elements and zero `<input>` elements** on a 504 KB site — the page exists, the booking does not. MetroDental: 16 buttons open a third-party Jotform in a new tab that warns the front desk will call to move weekend picks. Doncaster: a `pcurl.uk` shortlink, also a new tab, also off-domain. The test that separates all four from a real booking is the same one: does anything the visitor does put a slot in a calendar without a human touching it? Counting `<input>` on the destination page is a faster first pass than reading the copy.
- 2026-09-08 (step 4 — **the default logo pick was a stock photograph on two of three leads, and the gate cannot see it**): the standing warning is that `logo-not-a-mark` is invisible to `verify-brand.mjs` — a photo satisfies "a logo file is referenced and present". Today it was not an edge case: `metro-dental-makati/brand/logo-1-og-image.png` is a stock family portrait **and it is `logoFiles[0]`, the value the build script falls back to**, so building on defaults would have shipped it with all five checks green. `ora-dental-makati/brand/logo-2-og-image.png` is a stock patient portrait whose hexes "match" the captured golds because skin tone sits 55 away from #d7a86a in RGB — a numeric match that means nothing. Two things follow. Look at every candidate, not just the one you intend to use, because the next run inherits the ordering. And record the verdict where the build reads it: each `brand.json` now carries a `logoPick` field naming the vision-verified mark, and the build script resolves `lead.logoPick || brand.logoPick || logoFiles[0]`, so the correction survives in the file the gate reads instead of in a template comment.
- 2026-09-08 (step 4 — **"the recorded hexes are absent from the image" needs a number, and a monochrome mark is not a failed capture**): the demote-to-BRAND-BLOCKED rule turns on whether capture actually read the business, which by eye is a judgement call. Decoding the PNG and taking each recorded hex to its nearest opaque pixel makes it a measurement: Affinity #0654c4 → 25.5, MetroDental #00b8ff → 53.6 and #00d0e7 → 47.9, all comfortably inside the mark. Ora is the case the rule reads wrong at first glance — its wordmark is white on black, so the golds are **nowhere** in it, and only the blacks #0c0f1f/#191f3f match (27.7 / 31.1). That is not a bad capture: `colors.source` is `site-css`, so the palette never claimed to come from the logo, and the golds are genuinely the site's. Demote when the capture claims to have read a mark it plainly did not; do not demote a monochrome brand whose accent colour legitimately lives in its CSS.
- 2026-09-08 (step 4 — **the side-by-side contradiction block generalises past opening hours**): yesterday's KingTown page introduced "show both, declare neither", but the template had Friday/Saturday/Sunday nailed in as literal "Not published" rows and an "Opening hours" eyebrow, so it only worked for an hours conflict. Ora Dental's contradiction is two **addresses** — "1408 Kalayaan Ave, Poblacion" in its own page header against "Kalayaan Ave., cor. Salamanca" in the same page's footer — plus a phone number their landlord centuriamedical.com.ph publishes that they never do. Making the eyebrow and the trailing rows data rather than markup cost three lines and turned a one-lead block into the general shape: any two published sources, both shown, neither declared correct.
- 2026-09-08 (CLO-149 — **a backlog of "unbuilt but buildable" folders was 14 duplicate captures, and the state string is what hid it**): the issue read 12 folders at `ready:false` + `blockedBy:"palette-pending"` with a logo on disk and no `index.html`, and concluded, correctly per the table above, that each needed only a vision palette read. It was right about the rule and wrong about the folders. Eleven were a **second capture of a business that already had a canonical folder** — built, gated, live, and in most cases already in READY TO SEND. The proof is two comparisons, not a judgement: identical `facebook` handle **and** byte-identical logo file. `md5sum` matched exactly across all eleven pairs, e.g. `ivory-smile` / `ivory-smile-dental-makati` and `test-medika` / `medika-diagnostic-davao`. Had the run built them, every one of those prospects would have had two live URLs and a split outreach trail. `blockedBy` describes what capture found; it says nothing about whether the slug is the one the pipeline is using, so **check for a duplicate before believing a state string**.
- 2026-09-08 (CLO-149 — **"is there a card for it?" is the cheapest duplicate test, and an orphan folder is why nothing chased it for 19 days**): the standing re-capture pass walks cards in `BRAND BLOCKED`. These 12 folders had been parked since 2026-08-19 precisely because **none of them had a card anywhere** — a listing of all 40 BRAND BLOCKED cards contained not one of the 12 slugs, only the canonical `rjf-vulcanizing-taguig`. That is the tell. A folder with no card is not a lead waiting on work; it is a build artifact nothing points at, and repo-first scans manufacture backlog out of them. Run any repo-derived backlog against the board before assigning work to it: a slug that no card names is a cleanup, not a build.
- 2026-09-08 (CLO-149 — **the one genuine non-duplicate had the state wrong in the other direction**): `rjf-vulcanizing` was the only folder whose sibling was also unbuilt, so it was the only true candidate for a vision palette read. Opening the image settled it — a photograph of the shop interior, tyre racks and the vulcanising bay, no mark anywhere. So the correct state was `logo-not-a-mark`, the strictly *more* blocked one, and the config's own instruction for `palette-pending` ("vision-read the logo, write the hexes") would have invented a brand out of a snapshot of somebody's garage. `verify-brand.mjs` cannot catch this: the file is present, referenced and loadable, so all six checks pass. **A wrong `blockedBy` can under-state a block as easily as over-state one** — the vision read is what decides which, and it is the first step, not the last.
- 2026-09-08 (CLO-149 — **the two extra duplicates were the dangerous ones, because they were `ready:true`**): a full repo census turned up `mann-machine` and `tooth-doctors-davao` outside the issue's list — both `ready:true`, `blockedBy:null`, never built, and both duplicates (same site `mannandmachinevt.com` as `mann-machine-vt`; same FB handle `toothdoctorsdavao` as `tooth-doctors-dental`). These are worse than the twelve: they carry no blocker at all, so the next builder scanning for buildable work would have built them, passed the gate cleanly, and deployed a second live URL for a prospect already in the send queue. **Scan on "has no `index.html`", not on `blockedBy`** — the blocker field is exactly what a silently-duplicated folder is missing.
- 2026-09-08 (CLO-149 — **16 stale `blockedBy` strings were what made the repo scan untrustworthy in the first place**): 16 folders were `ready:true`, built, live and passing the gate while still carrying `blockedBy:"palette-pending"` from the 2026-08-19 retrofit. The field is only ever read by humans and by ad-hoc scans, so nothing broke — but it means any `blockedBy` census over the repo reports 28 blocked folders where 12 are stale-and-fine and the rest are duplicates. Cleared to `null`. A status field that no code reads still has to be true, because it is the field the next investigation will trust.
- 2026-09-08 (CLO-149 — **brand-only pushes are ungated by design, which is what makes a state correction safe to land**): 30 `brand.json` files changed in one push and the hook gated zero folders. That is correct, not a miss: `tools/hooks/pre-push` excludes `<slug>/brand/**` because it is builder *input*, not a deploy artifact. So a reconciliation pass that only rewrites capture state can never be blocked by an unrelated folder's build problem, and never risks changing what deploys. Keep state corrections confined to `brand/` and they land in one push; the moment one touches an `index.html`, the whole push is gated again.
- 2026-09-08 (step 6 QA, CLO-156 — **the shared skeleton this time was two different axes hiding in the same 3-card batch**): the day's only unmarked READY TO SEND cards were the 3 new Makati dental builds (Affinity, Ora, MetroDental). Reading each alone, all three read fine. Batch-wide, Affinity and MetroDental both opened their pain paragraph with a "When you `<verb>` …" subordinate clause — a shared opener skeleton — while Affinity and Ora both revealed the mockup with the identical construction `<subject clause> is/exists at URL — <feature>, <feature>, <feature>, and what it costs for `<X>`` — a shared reveal-sentence skeleton, the same defect class as the 2026-08-23 CTA-skeleton catch, just on a different sentence. Neither pair would have failed a card-by-card read. Rewrote Affinity's opener and reveal sentence to break both shared constructions; left MetroDental's opener and Ora's reveal as the surviving originals since a skeleton needs two, not one.
- 2026-09-08 (step 6 QA, CLO-156 — **the em-dash count is still being written into first drafts, not just recurring from an old generator**): Affinity's draft ran 3 em-dashes and Ora's ran 2, both well past the "zero or one" ceiling, while MetroDental — drafted in the same batch, same run — landed on exactly 1. There is no shared generator defect here; three drafts from one step-5 pass produced three different counts, which means the discipline is being applied inconsistently draft-to-draft rather than failing systematically. Cut every extra dash to a comma or a full stop in both drafts; no rewrite touched the underlying facts.
- 2026-09-08 (step 6 QA, CLO-156 — **batch result**): 3/3 new READY TO SEND cards (Affinity Dental, Ora Dental, MetroDental Greenbelt 5) failed humanize on first pass (em-dash count and/or a shared opener/reveal skeleton); all 3 rewritten in place and marked `✅ QA'd & humanized (2026-09-08)`. No other card in the 74-card READY TO SEND list was touched today before this run, so the batch-wide compare was scoped to just these 3. Brand gate independently re-run on all 3 slugs: 3/3 PASS, plus a visual open of each `logoPick` image against its recorded hexes — Affinity's blues, MetroDental's cyans and Ora's blacks (monochrome, `colors.source: site-css`, golds correctly not claimed from the logo) all genuinely match, confirming the 2026-09-08 step-4 vision notes rather than just trusting them. Deploy re-verified on content, not just status: 26842 / 28135 / 26289 bytes, each with the clinic's own `<title>`. Pricing: all 3 at PH ₱1,500/month top-of-band, first month free, six-month term stated as three separate facts with no computed total (the 2026-09-07 arithmetic-total defect did not recur). Menu boundary: 0/3 drafts named an automation ID or promised an `IN BUILD` capability — all correctly Tier-W-only.
- 2026-09-08 (CLO-150 close, CEO — **the approval question this file has cited for sixteen days was `expired`, not pending, and every run report said otherwise**): nine consecutive closes, yesterday's included, reported CLO-76's `ask_user_questions` as "pending since 2026-08-23, N days unanswered". Read directly today: interaction `1a17c02f` is **`expired`** and its predecessor `32d3012a` is `cancelled`; `POST .../withdraw` on the expired one returns `interaction_already_resolved`. An expired question is not a slow human, it is a question that stopped being asked — and it is indistinguishable from an unanswered one unless you read `status`. The list endpoint had also returned it as `pending` minutes earlier in this same run, so a single read is not enough either. Two rules follow. **Never report an interaction's age without re-reading its `status` in the same heartbeat**, and treat "pending for more than a few days" as a prompt to verify the interaction still exists rather than as evidence about the human.
- 2026-09-08 (CLO-150 close, CEO — **posting a new `ask_user_questions` retires the pending one on the same issue automatically**): creating `b8665a82` on CLO-76 left `1a17c02f` resolved without any withdraw call, and the explicit withdraw afterwards errored. So the supersede-and-replace pattern is one call, not two, and the second call fails in a way that reads like a permissions problem when it is actually success. Withdraw is only needed for a question on a *different* issue than the replacement.
- 2026-09-08 (CLO-150 close, CEO — **a standing human decision must not live on the day's run issue, because the run issue closes**): today's dispatch created the pipeline-direction question on CLO-150 itself at 03:17Z. CLO-150 closes `done` at the end of the same run, which would have left a live, human-only, wake-on-answer question sitting on a closed issue — inert by the same rule that makes agent comments on closed issues inert. Moved it to CLO-76, the standing `in_review` issue that already owns this blocker, and cancelled the CLO-150 copy. **Ask on the issue that outlives the run.**
- 2026-09-08 (CLO-150 close, CEO — **a one-call board census reconciles against the previous run's total, which recovers per-list depths the previous report never wrote down**): `trelloReadCard list_by_board limit=50` returned **124** nodes, `hasNextPage:false`, `nodes.length === totalCount`. Grouped client-side: NICHE 1 · BRAND BLOCKED 40 · READY TO SEND 74 · CHANGES REQUESTED 1 · SYNCED TO GHL 1 · REJECTED 7, and the other 8 lists empty — sum 124, so the roster reconciles exactly per the 08-31 rule. Yesterday's close recorded only the **total** (120) and READY TO SEND (71), yet the four small lists were unchanged at 10 between the two censuses, so BRAND BLOCKED on 09-07 is recoverable as 120 − 10 − 71 = **39**, which matches today's 40 minus the one card step 3 added (Doncaster). **Record the board total alongside the depth table** — it turns a later run's arithmetic into a check on both reports instead of a guess.
- 2026-09-08 (CLO-150 close, CEO — **step 9 contradicted its own evidence in its summary line, and the end-to-end number is the one that matters**): CLO-157 correctly observed that `[BAKERY] Custom Cakes by Bam` sits in SYNCED TO GHL (SEND MANUALLY) with `Sent: —`, then closed with "Sent to a prospect this run: 0; **ever: 1** (last on 2026-07-26)". A GHL sync is not a send; the card's own description says so, and 2026-07-26/27 is its sync date. The correct end-to-end line is **ever: 0**, unchanged since the pipeline started. The 09-06 rule asks for this line precisely because a table of healthy stage counts hides an end-to-end zero — it hides it just as well when the line itself is computed off the wrong list. **Count sends from CONTACTED and from a `Sent:` stamp, never from SYNCED TO GHL.**
- 2026-09-08 (CLO-150 close, CEO — **queue flags, stated exactly**): READY TO SEND grew for a **thirteenth consecutive run**: 17 → 23 → 35 → 39 → 42 → 48 → 49 → 54 → 57 → 60 → 64 → 71 → **74**, and has never once drained. BRAND BLOCKED is 38 → 38 → 39 → **40** — growth on the last two runs, flat on the one before, so it does **not** yet meet the three-consecutive-run flag; it trips next run if another `no-assets` lead lands, and it is worth naming now rather than discovering the streak retroactively. Every other watched queue is 0: INCOMING LEADS, STRATEGY READY, MOCKUP READY, APPROVED, CONTACTED, FOLLOW-UP DUE, REPLIED (HITS), CLIENTS. CHANGES REQUESTED 1 (CLO-75, Dei, 15 days) and SYNCED TO GHL 1 (Custom Cakes by Bam, 43 days unsent).
- 2026-09-08 (CLO-150 close, CEO — **an overdue due date on an unsent card is a real defect and it has now been reported twice without an owner**): CLO-157 flagged that Custom Cakes by Bam carries `due 2026-08-21T01:00:00Z` — 18 days overdue — while its own description still reads `Sent: —`, and this file's rule is that a due date is stamped only after a send. No agent can settle it, because only the founder knows whether the Facebook message was ever sent by hand. Rather than open a third copy of a question already asked, it is now the **second question inside CLO-76's live interaction** ("is the manual Facebook send actually something you can do, in practice?"), which is the same human touchpoint and the same answer. **Fold a single-card human question into the existing decision rather than opening an issue for it** — the escalation rule asks for one owner per blocker, not one issue per symptom.
- 2026-09-08 (CLO-150 close, CEO — **step 5's own verification table asserted a pass on the exact two checks step 6 then failed, which makes it an intention rather than a measurement**): CLO-155 closed with a "Content rules verified" block stating **"Em-dashes: 1 per draft ✓"** and a four-axis variation table presenting the opener and the reveal sentence as distinct across all three drafts. CLO-156, reading the same three cards an hour later, counted **3 em-dashes in Affinity and 2 in Ora** (only MetroDental was at 1), and found a **shared opener skeleton** between Affinity and MetroDental and a **shared reveal-sentence skeleton** between Affinity and Ora. So step 5's table was wrong on the em-dash count for two of three drafts and wrong on both of the axes it claimed to have varied. Nothing was lost — step 6 exists for this and caught all of it — but a self-check that reports a false pass costs step 6 the ability to treat step 5's output as a baseline, and on a day step 6 is skipped or thin it ships. **A step's verification block must quote the number it measured, not the rule it was aiming at**: "em-dashes: 3, 2, 1" is a measurement and would have failed itself; "1 per draft ✓" is a restatement of the ceiling. The same applies to a variation table — name the two constructions being compared, because "distinct" is exactly the judgement a card-by-card read gets wrong.
- 2026-09-12 (CLO-158 dispatch, CEO — **a Trello ARI contains TWO 24-hex ids and the card-id timestamp trick grabs the wrong one, dating every card on the board to 2021-11-26**): the 2026-09-06 learning says a Trello object id's first 8 hex chars are its creation time in Unix seconds, which is true of a bare MongoDB id. But `list_by_board` returns ids as ARIs — `ari:cloud:trello::card/workspace/61a0590fd5357d49f1237b01/6a7b9ffee330cf572210ee00` — and the **workspace** id comes first, so `id.match(/[0-9a-f]{24}/)` matches it and every card on the board dates to `2021-11-26T03:48Z`, the workspace's own creation. It is a plausible-looking date on every row, identical across all 124 cards, which is the only tell. Take the last path segment: `new Date(parseInt(id.split('/').pop().slice(0,8),16)*1000)`. Corrected, today's newest card reads `2026-09-08T03:22Z` (MetroDental) and the oldest BRAND BLOCKED card `2026-07-26T15:44Z` — which is what makes "the board has not moved since the 09-08 run" a measurement rather than an impression.
- 2026-09-12 (CLO-158 dispatch, CEO — **a byte-identical board census four days later is the cleanest example yet of "frozen, not stable", and the growth-streak flag reads it backwards**): every one of the fourteen list depths matched the 09-08 close exactly (124 total, BRAND BLOCKED 40, READY TO SEND 74, CHANGES REQUESTED 1, SYNCED TO GHL 1, REJECTED 7, NICHE 1, eight lists at 0) and the newest card on the board was four days old. By the config's literal rule — flag a queue that *grew* for a third consecutive run — READY TO SEND is fine and its 13-run streak is broken. It is not fine and the streak is not broken: the host slept 2026-09-09 05:15 (Kernel-Power 42) and resumed 2026-09-12 19:30, so the 09-09/10/11 runs never fired and nothing drafted. **A depth table needs the run count that produced it AND the host's uptime between the two measurements**; a queue cannot drain or grow while the machine is off, so an outage launders a growing queue into a stable-looking one for free.
- 2026-09-12 (CLO-158 dispatch, CEO — **re-asking a standing human question DESTROYS it, so the two rules about interactions have to be applied in order**): the 09-08 rules are (a) never report an interaction's age without re-reading `status` in the same heartbeat, and (b) posting a new `ask_user_questions` on an issue auto-retires the pending one. Applied together they mean the read comes first and the decision to re-ask is conditional on what it returns. CLO-76 read `32d3012a` cancelled / `1a17c02f` expired / **`b8665a82` pending, created 2026-09-08 04:15** — a genuinely live 4-day-old question. Restating it per run has not moved it, and the instinct after four silent days is to re-ask; doing so would have retired a real pending question and reset its clock to zero, converting a 4-day wait into a fresh one while looking like diligence. **Where the standing blocker's question is already `pending`, the correct action is nothing at all** — the escalation ladder is exhausted (CLO-76 `in_review`, CLO-75 `todo` 19 days, both owned) and a third copy is a regression, not a nudge.
- 2026-09-12 (step 2, CLO-160 — **a lead card's email came from a directory and belonged to a DIFFERENT clinic on a confusingly similar domain**): step 1 logged `cherry@cebudentalclinic.com` for Bright Smiles Oral Care Center, whose own site is `dentalclinic-cebu.com`. The two names are anagram-close and both are real, live, Google-MX'd .com registrations — so an MX check, a WHOIS age check and a plain 200 all pass on both and none of them separate the businesses. Fetching the page did: `cebudentalclinic.com` serves **Metro Cebu Oral Rehabilitation Center**, Gov. M. Roa St., Capitol Site — a different Cebu dental clinic on a different street, on a different host (67.227.253.107 / ns1.acmewebsolution.net vs 192.185.48.154 / ns8249.hostgator.com). An outreach draft written against that card would have pitched Bright Smiles' mockup to a competitor, from our own domain. **Resolving an email domain is not enough — read the page it serves and check the business name matches the lead.** The four-command DNS routine answers "does this host exist"; only the fetch answers "is this the same company".
- 2026-09-12 (step 2, CLO-160 — **the real email was on their own contact page all along, JS-cloaked, which is why a directory copy got used instead**): Bright Smiles' `/contact-us.html` renders "This email address is being protected from spambots" — the standard Joomla/WordPress cloak. A plain fetch, a regex for `\S+@\S+`, and a WebFetch summary all report no email on the page, which is what pushes the capture back onto an aggregator. The address is fully recoverable without a browser: the inline `document.getElementById('cloak<hash>')` script builds it from HTML entities, and de-entitising the two `addy<hash>` string concatenations yields `2008brightsmiles@gmail.com`. **Before recording "email not found" on a lead that has a site, grep its contact page for `cloak`, `addy`, `unescape` or `protected from spambots` and decode the script.** Same rule as the 2026-09-07 aggregator-published-it case, one layer deeper: the email was not missing, it was encoded.
- 2026-09-12 (step 2, CLO-160 — **an aggregator put its OWN homepage in the lead's "Website" field**): `cebudentalimplants.com/content/cebu-smiles-dentistry` lists the clinic's address, phone, email, Facebook and Instagram correctly, and then prints **"Website: https://cebudentalimplants.com/"** — the directory has installed itself as the clinic's web address on the page that ranks for the clinic's name, above six other clinics. This cuts both ways. As evidence it is the most checkable version of directory capture we have found: the owner clicks their own "website" link and lands back on the directory. As a trap it is the mirror of the 2026-09-07 "no owned site" error — a scraper that trusts a listing's Website field will record a lead as *having* a site when the value is the scraper's own source. **Treat a directory's "Website:" value as a claim about the directory until it resolves to a host the lead demonstrably controls.** Independently confirmed here: `cebusmilesdentistry.com` and `cebusmiles.com` are NXDOMAIN on A and MX and RDAP 404 at rdap.verisign.com — the clinic owns nothing.
- 2026-09-12 (step 2, CLO-160 — **a dead domain advertised in a Facebook PAGE TITLE, which is a place nobody thinks to resolve**): Cebu Docor Dental Clinic has three separate live Facebook pages, and one of them is named "Docor Dental Clinic Miller Hospital / **www.cebudocordental.com**". That domain does not exist — NXDOMAIN on A, MX *and* NS via dns.google, RDAP 404 at rdap.verisign.com, `curl` 000 — so every referral off their own page name hits a browser error and every mail to it bounces. The config's blank-domain checklist tells you to resolve the card's `Website:` field and to guess `<fb-slug>.com`; neither finds this, because the domain is in the page *name*, and the card's Website field said "Not found". **Read the Facebook page title and the About blurb as domain sources, not just the website field.** Two consequences for the offer: this is the strongest dead-domain evidence available, and **`BD-01` does not apply** — BD-01 is "the domain they are already paying for", and an unregistered name is a new purchase on the standard ladder, with no date promisable until they buy it.
- 2026-09-12 (step 2, CLO-160 — **a broken "book now" is the common case, but a real one does exist and must retire `CL-01` when it does**): the 09-08 learning catalogued four ways a booking surface is not a booking, and the standing instinct is now that `CL-01` always survives. It did not here. Rural View Dental's `/appointment/` has **zero `<form>` and zero `<input>` elements** — by the 09-08 `<input>`-count heuristic it reads exactly like Ora's fake `/book-now` — but it carries **one `<iframe>`**, pointed at `centaurportal.com/d4w/org-43/extended_search`, which returns a live Centaur D4W `bk9` booking app with practice / reason / provider selectors writing into their Dental4Windows calendar. Meanwhile Bright Smiles' "Book an Appointment" button, which *does* have 16 inputs, goes to a Joomla contact form with **no date field and no time field** and books nothing. So the input count is a fast first pass and it is wrong in both directions: **count `<iframe>` as well as `<input>`, and check whether the inputs include a date and a time.** Pitching a clinic an automation it already runs is worse than pitching nothing.
- 2026-09-12 (step 2, CLO-160 — **a pasted Chrome address bar was live on an intl lead's homepage, which is a new and very strong angle**): two clickable elements on ruralviewdental.com.au's homepage — the "We are an Accredited Dental Practice" panel and the **"4.9 | 145 Reviews"** button, their entire social-proof CTA — both carry `href="https://www.google.com/search?q=rurAl+view+dental+australia&rlz=1C5GCEA_enID1170ID1170&oq=rura&gs_lcrp=…&source=chrome.rb"`. Somebody built the page with their browser's URL bar in the clipboard: the query is typo'd (`rurAl`), and the `rlz` tag is an Indonesia-locale Chrome install, i.e. the contractor's machine, not the client's. The effect is that the clinic's reviews button hands its visitor to Google's results page for its own name, where **Northern Beaches Dental — 1-3 Old Eimeo Rd, the next street over from their 10 Eimeo Rd — takes 7 of the first 10 results.** This is name collision the owner inflicted on themselves, checkable in one click, and it beats the directory-capture default outright on a lead that already has a site. **Grep a lead's own HTML for `google.com/search`, `rlz=`, `utm_` and `source=chrome` before settling on an angle** — a hardcoded search URL, an analytics link or a preview-domain `og:url` (09-08) are all the same class of defect: the site telling the world its canonical address belongs to somebody else.
- 2026-09-12 (step 2, CLO-160 — **`.au` RDAP was unreachable and that is "could not connect", not "no RDAP"**): the 09-08 rule says never read an empty body as "this TLD has no RDAP". The same care applies one step earlier. `rdap.auda.org.au` and `rdap.audns.net.au` both returned **curl code 000** — a connection failure, no HTTP transaction at all — and `rdap.identitydigital.services` returned a well-formed 404 because it is the wrong registry, not because the domain is unregistered. Three different failures, three different meanings, and all three would have collapsed into "no result" in a report that only logged "RDAP failed". The registration date for the AU lead is therefore **unverified and was left off the card** rather than guessed or quietly dropped. **Log the transport outcome per endpoint — 000 vs 404 vs 200-with-empty-body — because only the third one is a statement about the TLD.**
- 2026-09-12 (step 2, CLO-160 — **the ~1,250-char strategy budget does not survive a card that has to carry a correction**): all four cards landed at 1,313 / 1,351 / 1,364 / 1,399 characters, leaving step 5 between **649 and 735** chars rather than the ~800 the eviction rule assumes — and that is *after* compressing step 1's Lead Details prose down to a two-line header, which is eviction order #2 and the only reason they fit at all. The cause is structural, not sloppy writing: three of the four cards had to spend 150-200 chars documenting a corrected contact fact (`CONTACT FIX:` lines), because dropping the correction re-introduces the wrong email on the next read. **A card carrying a contactability correction should be budgeted at ~1,400, not ~1,250**, and the step that writes it must say in its report which downstream step got squeezed and by how much — the 2026-08-23 GMS failure was a silent overrun, and an unannounced 649-char draft budget is the same defect discovered one step later.
- 2026-09-12 (step 4, CLO-162 — **gate check 5 was inert or checking the wrong string on all three folders in the batch, because `brand.name` is a scraped `<title>`, not a name**): the check does `name.split(/\s+[-|]\s+/)[0]` and requires part one verbatim in `index.html`. Three folders, three different failures of that assumption. `bright-smiles` was captured as `"Dental Clinic Cebu City, Philippines - Bright Smiles Oral Care Center"`, so the check was policing the SEO prefix and **never once looked for the business name**. `rural-view-dental` was `"Rural View Dental &#8211; Great dental health starts here"` — an en-dash *entity*, which is not a hyphen, so nothing split, the required substring became the whole tagline, and the folder would have **failed on a name that is on the page**. `cebu-smiles-dentistry` was `null`, because a Facebook-only lead has no site and so no title to scrape, and a null name makes the check **skip in silence** — the one check that proves the prospect's own name reached the page was not running at all for the most common lead type in this pipeline. Fix is in `brand.json`, not the gate: `name` holds the business name and the raw title moves to a new `siteTitle` field. **Capture should be setting `name` from the mark or the card, and treating the `<title>` as a hint; until it does, step 4 must read `brand.name` and ask whether it is a name.**
- 2026-09-12 (step 4, CLO-162 — **a framework's default palette is indistinguishable from a captured brand palette, and the ">=2 captured hexes" check certifies it**): `rural-view-dental`'s site-CSS sweep returned `#6abd44` (correct) followed by `#d9534f`, `#5cb85c`, `#cc3366` and `#ff6900` — Bootstrap danger/success and Elementor defaults, all genuinely present in the stylesheet and therefore all genuinely "captured". The template takes slots 1-3 as primary/secondary/accent, so the page would have shipped a **Bootstrap-red accent on a green-and-navy practice** and passed the gate 6/6. Worse, the colour that is **35.1% of their actual logo** — the navy `#2c3691` — was absent from `colors.brand[]` entirely, because Elementor never emits it as a literal hex. Same class of hole on `bright-smiles`: slots 1 and 2 were `#0db14b` and `#0db54c`, two greens **4/255 apart**, which the check happily counts as two distinct brand colours while rendering a monochrome page. **The gate counts hexes; it cannot tell a brand from a boilerplate, a palette from one colour listed twice, or notice an absence. Rank `colors.brand[]` against the logo before building, and treat a Bootstrap/Elementor hex in the top three as a capture miss.**
- 2026-09-12 (step 4, CLO-162 — **"vision-read the logo" can be *measured*, headlessly, with zero dependencies — stop eyeballing hexes**): the config's palette-pending path says vision-read the mark, and in practice that has meant estimating. Estimates are wrong in a specific direction: step 3 recorded `cebu-smiles`'s accent as `#ff1493`, which is exactly the CSS keyword `deeppink`, and its frame as `#7030a0`. Actual sampled values are `#ed008b` and `#672d91` — the eye reaches for the nearest *named* colour. Both formats are decodable in this environment. **PNG:** `zlib.inflateSync` + the five PNG unfilter cases is ~30 lines of plain Node and gives an exact pixel histogram (that is how the `#2c3691` navy above was found, at 35.1%, and how `#0db14b` was confirmed at 35.0% against the CSS). **JPEG:** PowerShell `Add-Type -AssemblyName System.Drawing` → `New-Object System.Drawing.Bitmap` → `GetPixel`, no browser and no image library. Watch one trap in the PS version: quantising by `[Math]::Round($c.R/8)*8` produces 256 for a white channel and the `{0:x2}` format silently emits a **nine-character** "hex" like `#100100100`. Sample the raw channel values, or clamp. `colors.source` stays `"vision"` either way — it is still vision, just measured instead of guessed.
- 2026-09-12 (step 4, CLO-162 — **the only typeface a site "captures" can be its `<code>` fallback**): `bright-smiles` came back with exactly one font, `Consolas`, which clears every filter the gate has — it is over 3 characters, it is not in `GENERIC_FONTS`, it is not a CSS property — so the gate would have certified a dental clinic set in a programmer's monospace as "typeface used". It appears in `theme.css` beside the theme's own comment *"Consolas has a better baseline in running text compared to `Courier`"*: it is the `<code>`/`<pre>` fallback in a normalize block. The two real families in the same stylesheet are `Istok Web` and `Open Sans`. **A single captured font that is a known monospace (Consolas, Menlo, Monaco, ui-monospace, SFMono) is a capture miss, not a brand face — go back to the stylesheet and read the `body`/heading declarations before you let the gate bless it.**
- 2026-09-12 (step 4, CLO-162 — **the contradiction block was hard-coded to two sources and the real world had three**): the 09-07 template's `hoursConflict` is `Day | source A | source B`, and the config already generalised its *use* to addresses and phone numbers. But Cebu Smiles has **three** phone numbers in circulation — the card's `0917 165 2352`, the SM mall directory's `(032) 412-7023`, and an aggregator's `0933 325 7820` — and two columns cannot show three sources without silently dropping one, which is the exact dishonesty the block exists to avoid. Generalised to `sideBySide` with an N-column `--cols` custom property in `tools/build-mockups-2026-09-12.mjs`; the column count is now data. **When a template constant encodes "how many of these there usually are", it will eventually force the page to lie by omission.**
- 2026-09-12 (step 4, CLO-162 — **the branded domain and `pages.dev` do not serve byte-identical pages, and the difference is Cloudflare rewriting every `mailto:` on the page**): `pages.dev` returned exactly the committed file for all three mockups (26,894 / 28,332 / 27,596 — byte-for-byte the local build). `preview.cloudspringitsolutions.com` returned **+310 to +315 bytes each** (27,209 / 28,638 / 27,857). The delta is Scrape Shield's Email Address Obfuscation: each `<a href="mailto:...">` becomes `<a href="/cdn-cgi/l/email-protection#<hex>">` wrapping `<span class="__cf_email__" data-cfemail="<hex>">[email&#160;protected]</span>`, plus one injected `/cdn-cgi/scripts/.../email-decode.min.js`. It is XOR-with-the-first-byte and all three **decode back to the correct address**, so the link works for any visitor with JS — but a no-JS visitor reads the literal string "[email protected]" where the page's whole argument is *this is the inbox that receives*. Two consequences. **(1) The byte-count deploy check is a threshold test, not an equality test** — 514 means placeholder and tens of thousands means live, but do not expect the two hosts to agree, and do not read a few hundred bytes of drift as a stale deploy. **(2) This applies to every mockup that prints an email**, which is most of them; it is a Cloudflare dashboard setting (Scrape Shield → Email Address Obfuscation), not anything a push can change, so it is a human call whether to turn it off.
- 2026-09-12 (step 6, QA, CLO-164 — **a closer can pass every hard check and still not track its own draft, and none of the four checks catch it**): Cebu Smiles Dentistry's closer read "Which of those three numbers should ring straight to you?" — "ring straight to you" is telephone idiom, and the draft names exactly one phone number, so a reader has to reverse-engineer that "three numbers" actually means price / free-month / term, three facts the draft never called "numbers" at all. Zero em-dashes over the limit, contractions present, no banned phrase, no arithmetic violation — it clears every rule in the humanize section and still reads like a fragment left over from a different draft. **A closer has to refer back to something the draft said, in words the reader can trace, not just a countable quantity that happens to match.** Rewrote to "Which matters most to you: the price, the free month, or the term?" — same choice-question construction, same sign-off, now traceable. Add this as a fifth thing "humanize" checks for, because the other four are all measurements and this one is a comprehension test that only a full read catches.
- 2026-09-12 (step 6, QA, CLO-164 — **batch result**): today's step-5 output was exactly 3 cards (Cebu Smiles Dentistry, Bright Smiles Oral Care Center, Rural View Dental Mackay AU) — the other 74 READY TO SEND cards were already marked and untouched by step 5 today, so the batch-wide compare was correctly scoped to these 3. Em-dash count independently re-measured at 1 / 0 / 0, matching step 5's own count exactly (first time in three QA runs step 5's self-reported number has been right). Sign-off constructions ("Thanks, Dei" / "Let's talk, Dei" / "Talk soon, Dei"), reveal sentences, and CTA orderings independently verified as genuinely distinct across all three — no shared skeleton found on any of the four axes. Arithmetic check: none of the three state a computed contract total alongside a free-month claim. Brand gate re-run positionally (not `--slug`) on all 3: 3/3 PASS, 6/6 each. Visually opened each `logoPick`/logo file against its recorded hexes: Cebu Smiles' purple/magenta badge matches `#672d91`/`#ed008b`, Bright Smiles' green rounded-rect matches `#0db14b`, Rural View's script wordmark visibly carries both the green and the corrected navy `#2c3691` — all three confirm the step-4 vision notes rather than just trusting them. Deploy re-verified on content: pages.dev 26894/28332/27596 bytes, branded host 27209/28638/27857 (matching the step-4 Scrape Shield delta exactly, so the drift is the known mailto-obfuscation effect and not a stale deploy), each with the correct per-business `<title>`. Pricing: 2 PH cards at ₱1,500/month top-of-band with price/free-month/term stated as separate facts (no computed total); 1 AU card at AUD 650 build + AUD 110/month, both inside the OFFER-MENU FX band (475–700 / 80–140) and the rate stamp (2026-08-18) is 25 days old, inside the 30-day refresh window. Menu boundary: 0/3 drafts named an automation ID or an `IN BUILD` capability — all correctly Tier-W-only. One rewrite (Cebu Smiles closer, above); all 3 marked `✅ QA'd & humanized (2026-09-12)`.
- 2026-09-12 (CLO-158 close, CEO — **the growth flag only means something for a queue that has a drain, and BRAND BLOCKED does not have one**): BRAND BLOCKED went 39 → 40 → **41** and so trips the config's "grew for a third consecutive run" flag for the first time, exactly as the 09-08 close predicted it would. Reporting it as a bottleneck is still close to meaningless, and the reason is structural: a `no-assets` lead enters BRAND BLOCKED and *nothing in the nine steps ever reads that list again*. It has no drain, by design. A queue with no drain trips a growth flag eventually no matter how the pipeline is performing, so the flag carries no information about it — whereas READY TO SEND's streak is informative precisely because READY TO SEND *has* a drain (the human approval gate) that is not being used. **Apply the growth flag only to queues with a drain; for a drainless queue the meaningful question is the stock, not the slope.** The stock here is 41 cards — 32% of the whole 128-card board, oldest 2026-07-26 — representing leads permanently parked on a capture failure, and that is a backlog question for a human, not a per-run flag.
- 2026-09-12 (CLO-158 close, CEO — **the fastest way to hit the 514-byte placeholder is to guess the slug yourself**): the config already says 514 bytes means placeholder and tens of thousands means live. What it does not say is what most often *causes* a placeholder hit during verification. Checking today's three deploys I asked for `/bright-smiles/` — abbreviating the business name the way a slug usually reads — and got **HTTP 200, 514 bytes, `<title>CloudSpring IT Solutions — Previews</title>`**: a success code, a plausible title, and a page that is not the mockup. The real slug is `bright-smiles-oral-care-center` and it serves 28,638 bytes with the correct per-business title. For one anxious minute that read as a build that had silently failed to deploy. **Never compose a slug from the business name — read it from `git ls-tree --name-only origin/main` or from the card's own `MOCKUP:` line**, because the placeholder answers every wrong slug with a 200 and a title that looks like a real page, so a slug typo and a dead deploy are indistinguishable from the status code and nearly so from the title.
- 2026-09-12 (CLO-158 close, CEO — **MCP connector access is per-AGENT, not per-company, and the step that has none reported queue depths anyway**): CLO-165 (steps 7-9) wrote "Cannot re-measure without Trello authentication" and "Cannot query GHL inbound without GHL MCP authentication" — then reported APPROVED 0, FOLLOW-UP DUE 0 and CONTACTED 0 as results. Those three numbers were not measurements; they were the dispatch brief restated back. Re-measured independently from the CEO seat, where the Trello connector *is* authorised, all three are genuinely 0, so nothing was lost today — the step was right by agreement, not by evidence. The failure mode is one-directional and arrives on the first good day: the moment a human actually approves a card, step 7 will report "APPROVED 0" in the same confident format and the card will sit there unsent. Two fixes, both cheap. **(1) A step that cannot read a queue must write `not measured (no connector)`, never a number** — the standing rule is that a step which produced nothing still reports it, and "0" and "unmeasurable" are different reports. **(2) Queue depths in the run report are the CEO's measurement, taken at close from the seat that has board access**, not a value collected from the steps. Worth noting the connector split is invisible from the dispatch side: the CEO agent sees Trello fine, so a brief written here silently assumes an access the executing agent lacks.
- 2026-09-12 (CLO-158 close, CEO — **a step posting the same report three times in twelve seconds, the first copy mojibaked**): CLO-165 carries three near-identical comments at 12:41:22, 12:41:28 and 12:41:34. The first renders as `Steps 7�9` and `Step 7 � Approved handling` — cp1252 replacement characters where em-dashes and en-dashes belong — the second is the same text rewritten in pure ASCII (`Steps 7-9`), and the third restores real Unicode dashes and reads correctly. So the agent detected its own encoding damage and retried twice, which is the right instinct, but left all three copies on the thread with no marker saying which is authoritative. A later reader reconstructing the run from comments sees three reports and no way to tell them apart except by timestamp. **When a post is retried for encoding, the superseded copies need deleting or an explicit "superseded" line** — and the ASCII-only rule already in this file for PowerShell scripts applies to API comment bodies too, since the same cp1252 path mangles both.
- 2026-09-12 (CLO-158 close, CEO — **the standing question's own text has gone stale, and that is not a reason to re-post it**): CLO-76's interaction `b8665a82` is still `pending` at 4.3 days (created 2026-09-08 04:15Z, re-read this heartbeat per the 09-08 rule, not carried forward). Its prompt says "READY TO SEND is at **74** QA-passed drafts and APPROVED has been 0 for 13 straight runs"; the true figures at close today are **77** and 14. The instinct is to refresh the question so the founder sees current numbers — and doing so would auto-retire the live 4-day-old question and reset its clock, per the 09-08 supersede rule, trading a real pending ask for a cosmetically fresher one. **Report the drift in the run report instead; a pending question's stale numbers are a reporting problem, not an interaction problem.** The question being asked — keep filling the queue or change the model — is not affected by whether the queue reads 74 or 77, which is itself the answer to whether re-asking would add anything.
